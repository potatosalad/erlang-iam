...
encode(Input, Opts) when ?is_iodata(Input) andalso is_map(Opts) ->
	...
	H = << << (?INT_TO_B64URL(V)) >> || << V:6 >> <= Head >>,
	{T, Pad} =
		case Tail of
			<< T0:6, T1:6, T2:4 >> ->
				{<< ?INT_TO_B64URL(T0), ?INT_TO_B64URL(T1), ?INT_TO_B64URL(T2 bsl 2) >>, << $= >>};
			<< T0:6, T1:2 >> ->
				{<< ?INT_TO_B64URL(T0), ?INT_TO_B64URL(T1 bsl 4) >>, << $=, $= >>};
			<<>> ->
				{<<>>, <<>>}
		end,
	case Padding of
